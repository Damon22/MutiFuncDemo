...
-(void)downloadWithUrlString:(NSString *)urlString toPath:(NSString *)destinationPath process:(ProcessHandle)process completion:(CompletionHandle)completion failure:(FailureHandle)failure
{
    if(urlString&&destinationPath)
    {
        ...
        if(fileExist)
        {
            ...
            // 设置请求头
            NSString *rangeString=[NSString stringWithFormat:@"bytes=%zd-",length];
            [request setValue:rangeString forHTTPHeaderField:@"Range"];
        }
        _con=[NSURLConnection connectionWithRequest:request delegate:self];
        // 更新NSURLSession
        _urlSection = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration] delegate:self delegateQueue:[[NSOperationQueue alloc] init]];
        
        // 创建流
        NSOutputStream *stream = [NSOutputStream outputStreamToFileAtPath:destinationPath append:YES];
        
        // 创建一个Data任务
        NSURLSessionDataTask *task = [_urlSection dataTaskWithRequest:request];
