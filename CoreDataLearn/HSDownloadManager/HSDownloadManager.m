...
{
    CGFloat fileProgress = [self fileTotalLength:url] == 0 ? 0.0 : 1.0 * HSDownloadLength(url) /  [self fileTotalLength:url];
    return fileProgress;
}
...
{
    NSInteger fileTotal = [[NSDictionary dictionaryWithContentsOfFile:HSTotalLengthFullpath][HSFileName(url)] integerValue];
    return fileTotal;
}
 ...
 *  @param url  取消的地址
 */
- (void)cancleDownloadForUrl:(NSString *)url
{
    [self pause:url];
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data
{
    ...
    if(freeSpace<1024*1024*20){
        if ([[UIDevice currentDevice].systemVersion floatValue]>=9.0) {
            ...
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"提示" message:@"系统可用存储空间不足20M" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
        }
