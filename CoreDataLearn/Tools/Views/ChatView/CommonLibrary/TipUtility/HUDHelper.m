...
{
    [self stopLoading:hud message:msg delay:0 completion:^{
        [_syncHUD removeFromSuperViewOnHide];
        _syncHUD = nil;
        _instance = nil;
    }];
}
...
{
    [self tipMessage:msg delay:seconds completion:^{
        [_syncHUD removeFromSuperViewOnHide];
        _syncHUD = nil;
        _instance = nil;
    }];
...
- (void)tipMessage:(NSString *)msg delay:(CGFloat)seconds completion:(void (^)())completion
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:[BaseAppDelegate sharedAppDelegate].window];
...
{
    [self syncStopLoadingMessage:nil delay:0 completion:^{
        [_syncHUD removeFromSuperViewOnHide];
        _syncHUD = nil;
        _instance = nil;
    }];
...
{
    [self syncStopLoadingMessage:msg delay:1 completion:^{
        [_syncHUD removeFromSuperViewOnHide];
        _syncHUD = nil;
        _instance = nil;
    }];
