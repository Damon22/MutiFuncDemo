...
#define DMSwitchMaxHeight 31.0f
#define DMSwitchMinHeight 31.0f

#define DMSwitchMinWidth  51.0f
#define DMSwitchKnobSize  27.0f
...
- (void)setOnTintColor:(UIColor *)onTintColor
{
    if (_onTintColor != onTintColor) {
        ...
        if (_style == DMSwitchStyleNoBorder) {
...
- (void)setStyle:(DMSwitchStyle)style
{
    if (_style != style) {
        _style = style;
...
- (void)layoutSubviews
{
    ...
    CGFloat margin = (CGRectGetHeight(self.bounds) - DMSwitchKnobSize) / 2.0;
    ...
    if (!self.isOn) {
        ...
        self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - margin - DMSwitchKnobSize,
                                         margin,
                                         DMSwitchKnobSize,
                                         DMSwitchKnobSize);
    }
    ...
    self.onLabel.frame = CGRectMake(lMargin,
                                    r - lHeight / 2.0,
                                    CGRectGetWidth(self.onContentView.bounds) - lMargin - DMSwitchKnobSize - 2 * margin,
                                    lHeight);
    
    self.offLabel.frame = CGRectMake(DMSwitchKnobSize + 2 * margin,
                                     r - lHeight / 2.0,
                                     CGRectGetWidth(self.onContentView.bounds) - lMargin - DMSwitchKnobSize - 2 * margin,
                                     lHeight);
}
...
- (void)commonInit
{
    ...
    _borderView.layer.borderColor = _on ? _onTintColor.CGColor : _tintColor.CGColor;
    _borderView.hidden = (_style == DMSwitchStyleNoBorder) ? YES : NO;
    [_containerView addSubview:_borderView];
    
    _onContentView = [[UIView alloc] initWithFrame:self.bounds];
    _onContentView.backgroundColor = (_style == DMSwitchStyleNoBorder) ? _onTintColor : [UIColor clearColor];
    [_containerView addSubview:_onContentView];
    
    _offContentView = [[UIView alloc] initWithFrame:self.bounds];
    _offContentView.backgroundColor = (_style == DMSwitchStyleNoBorder) ? _tintColor : [UIColor clearColor];
    ...
    _knobView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, DMSwitchKnobSize, DMSwitchKnobSize)];
    _knobView.backgroundColor = (_style == DMSwitchStyleNoBorder) ? _thumbTintColor : (_on ? _onTintColor : _tintColor);
    _knobView.layer.cornerRadius = DMSwitchKnobSize / 2.0;
...
- (CGRect)roundRect:(CGRect)frameOrBounds
{
    ...
    if (newRect.size.height > DMSwitchMaxHeight) {
        newRect.size.height = DMSwitchMaxHeight;
    }
    
    if (newRect.size.height < DMSwitchMinHeight) {
        newRect.size.height = DMSwitchMinHeight;
    }
    
    if (newRect.size.width < DMSwitchMinWidth) {
        newRect.size.width = DMSwitchMinWidth;
    }
...
- (void)scaleKnodViewFrame:(BOOL)scale
{
    ...
    if (self.isOn) {
        self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - DMSwitchKnobSize - margin - (scale ? offset : 0),
                                         margin,
                                         DMSwitchKnobSize + (scale ? offset : 0),
                                         DMSwitchKnobSize);
    } else {
        self.knobView.frame = CGRectMake(margin,
                                         margin,
