...
#import "NSDate+JXCNewsAddition.h"

static int localAndServerGap = 0;

@implementation NSDate (JXCNewsAddition)
...
- (NSString*)transformToFuzzyDate
{
    ...
    else if (timeInterval < 24 * 60 *60 * 8)
    {
        NSString* dateString = [NSString stringWithFormat:@"%u天前",(unsigned int)(timeInterval/(24 * 60 * 60))];
        return dateString;
    }
...
- (NSString*)promptDateString
{
    ...
    NSCalendar *greCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    //  通过已定义的日历对象，获取某个时间点的NSDateComponents表示，并设置需要表示哪些信息（NSYearCalendarUnit, NSMonthCalendarUnit, NSDayCalendarUnit等）
    NSDateComponents *nowDateComponents = [greCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | kCFCalendarUnitWeek | NSCalendarUnitWeekday | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekOfYear fromDate:nowDate];
    
    NSDateComponents *selfDateComponents = [greCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | kCFCalendarUnitWeek | NSCalendarUnitWeekday | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekOfYear fromDate:self];
    ...
    else if (selfTnterval < 2 * 60 * 60 && nowDateComponents.day == selfDateComponents.day)
    {
        NSString* dateString = [NSString stringWithFormat:@"%u小时前",(unsigned int)(selfTnterval / (60 * 60))];
        return dateString;
    }
    else if ((nowDateComponents.weekOfMonth == selfDateComponents.weekOfMonth) && (nowDateComponents.month == selfDateComponents.month) && (nowDateComponents.year == selfDateComponents.year))
    {
        //在同一个周中
